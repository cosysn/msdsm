...
--*/
{
    ...
    TracePrintEx("DsmpAddDeviceEntry (Group %p, DevInfo %p, DevNum %s): Entering function.\n",
				Group,
                DeviceInfo,
    ...
    if (numberDevices < DSM_MAX_PATHS) {
        ...
        TracePrintEx("DsmpAddDeviceEntry (Group %p, DevInfo %p, DevNum %s): Adding Device to Group %p\n",
					Group,
					DeviceInfo,
					DeviceInfo->SerialNumber,
                    Group);
        ...
        TracePrintEx("DsmpAddDeviceEntry (Group %p, DevInfo %p, DevNum %s): Max Paths already added for Group %p.\n",
					Group,
					DeviceInfo,
					DeviceInfo->SerialNumber,
                    Group);
    ...
    TracePrintEx("DsmpAddDeviceEntry (DevInfo %p, DevNum %s): Exiting function with status %x.\n",
                DeviceInfo,
				DeviceInfo->SerialNumber,
                status);
...
--*/
{
	...
	/*
    TracePrintEx("DsmpFindFOGroup (PathId %p): Entering function.\n",
                PathId);
				*/
    //
    ...
    for (i = 0; i < DsmContext->NumberFOGroups; i++, entry = entry->Flink) {
        ...
        if (failOverGroup->PathId == PathId) {
			...
			/*
            TracePrintEx("DsmpFindFOGroup (PathId %p): Found a FO group %p.\n",
                        PathId,
                        failOverGroup);
						*/
	...
	/*
    TracePrintEx("DsmpFindFOGroup (PathId %p): Exiting function with retFOGroup %p.\n",
                PathId,
                retFOGroup);
				*/
    return retFOGroup;
...
--*/
{
    ...
    TracePrintEx("DsmpBuildFOGroup (PathId %p): Entering function.\n", PathId);
    ...
    if (failOverGroup) {
        ...
        TracePrintEx("DsmpBuildFOGroup (PathId %p): Path that will be used for %p is %p.\n",
                    PathId,
                    DeviceInfo,
                    *PathId);
        ...
        TracePrintEx("DsmpBuildFOGroup (PathId %p): Added new FOGroup %p with path %p. Count of FO Group %d.\n",
                    PathId,
                    failOverGroup,
                    *PathId,
                    DsmContext->NumberFOGroups);
        ...
        TracePrintEx("DsmpBuildFOGroup (PathId %p): Failed to allocate memory for FailOverGroup.\n",
                    PathId);
    }

    TracePrintEx("DsmpBuildFOGroup (PathId %p): Exiting function with failOverGroup %p.\n",
                PathId,
                failOverGroup);
...
--*/
{
    ...
    TracePrintEx("DsmpRemoveDeviceEntry (Group %p, DevNum %s): Entering function. DeviceInfo %p.\n",
                Group,
				DeviceInfo->SerialNumber,
                DeviceInfo);
    ...
    for (i = 0; i < Group->NumberDevices; i++) {
        ...
        if (Group->DeviceList[i] == DeviceInfo) {
            ...
            TracePrintEx("DsmpRemoveDeviceEntry (Group %p, Devnum %s): Removing Device %p (desiredState %u) from Group\n",
                        Group,
						DeviceInfo->SerialNumber,
                        DeviceInfo,
    ...
    if (!freeGroup && Group->ReservationList) {
        ...
        for (i = 0; i < Group->NumberDevices; i++) {
            ...
            if (Group->DeviceList[i]->RegisterServiced) {
                TracePrintEx("DsmRemoveDeviceEntry (Group %p): Device %p at %d registered.\n",
                            Group,
                            Group->DeviceList[i],
                            i);
        ...
        TracePrintEx("DsmRemoveDeviceEntry (Group %p): Reservations Old (%x) New (%x).\n",
                    Group,
                    oldList,
    ...
    TracePrintEx("DsmpRemoveDeviceEntry (Group %p): Exiting function - numberDevices = %x.\n",
                tempAddress,
                numberDevices);
...
--*/
{
    ...
    TracePrintEx("DsmpSetNewPath (DevInfo %p, DevNum %s, DevState: %d): Entering function.\n",
                FailingDevice, FailingDevice->SerialNumber, FailingDevice->State);
    ...
    TracePrintEx("DsmpSetNewPath (DevInfo %p, DevNum %s, DevState: %d): Exiting function with path (failGroup) %p.\n",
                FailingDevice,
				FailingDevice->SerialNumber,
				FailingDevice->State,
...

{
    ...
    TracePrintEx("DsmpSetNewPathUsingGroup (Group %p): Entering function.\n",
                Group);
    ...
    if (pDevInfo == NULL) {
        TracePrintEx("DsmpSetNewPathForZombieGroup (ZombieGroup %p): No failover group can be found.\n",
                    Group);
        return NULL;
    }
    ...
    TracePrintEx("DsmpSetNewPathForZombieGroup (ZombieGroup %p): Exiting function with path (failGroup) %p.\n",
                Group,
                Group->PathToBeUsed);
...
--*/
{
	...
	/*
    TracePrintEx("DsmpGetPath (DsmIds %p): Entering function.\n",
                DsmList);
				*/
    if (!(DsmList->Count && deviceInfo)) {
		/*
        TracePrintEx("DsmpGetPath (DsmIds %p): Called with no available paths.\n",
                    DsmList);
					*/
...
/*
				TracePrintEx("DsmpGetPath (DsmIds %p, DevNum %s): <TargetPortGroup> = %p <IsDeviceFailedState> = %d.\n",
					DsmList,
					deviceInfo->SerialNumber,
					deviceInfo->TargetPortGroup,
					DsmpIsDeviceFailedState(deviceInfo->State)
				);

				TracePrintEx("DsmpGetPath (DsmIds %p, DevNum %s): <LoadBalanceType> = %d <State> = %d <DesiredState> = %d <ALUAState> = %d.\n", 
					DsmList,
					deviceInfo->SerialNumber,
...
*/
 ...
 /*                       TracePrintEx("DsmpGetPath (DsmIds %p, DevNum %s): Candidate device %p.\n",
                                    DsmList,
									deviceInfo->SerialNumber,
                                    candidateDevice);
*/
  ...
  /*          TracePrintEx("DsmpGetPath (DsmIds %p, DevNum %s): Path to be used is %p.\n",
                        DsmList,
						deviceInfo->SerialNumber,
  ...
  /*                  TracePrintEx("DsmpGetPath (DsmIds %p, DevNum %s): New Path is %p.\n",
                                DsmList,
								deviceInfo->SerialNumber,
        ...
        case DSM_LB_DYN_LEAST_QUEUE_DEPTH: {
            ...
            if (failGroup) {
				/*
                TracePrintEx("DsmpGetPath (DsmIds %p, DevNum %s): Path to be used for LQD is %p.\n",
                            DsmList,
							deviceInfo->SerialNumber,
                            failGroup);
							*/
                ...
                if (!DsmpIsSymmetricAccess((PDSM_DEVICE_INFO)DsmList->IdList[0])) {
					...
					/*
                    TracePrintEx("DsmpGetPath (DsmIds %p, DevNum %s): Using same path (FOG %p) as previous request for LQD.\n",
                                DsmList,
								deviceInfo->SerialNumber,
                                failGroup);
								*/
					...
					/*
                    TracePrintEx("DsmpGetPath (DsmIds %p, DevNum %s): Failed to find a path for LQD.\n",
                                DsmList,
								deviceInfo->SerialNumber);
								*/
                }
        ...
        case DSM_LB_LEAST_BLOCKS: {
            ...
            if (isRead || isWrite) {
                ...
                if (groupEntry->UseCacheForLeastBlocks) {
                    ...
                    if ((lastPathUsed != NULL) &&
						...
						/*
                        TracePrintEx("DsmpGetPath (DsmIds %p, DevNum %s): Sequential IO, so using same path %p for LeastBlocks.\n",
                            DsmList,
							deviceInfo->SerialNumber,
                            failGroup);
							*/
            ...
            if (failGroup) {
				/*
                TracePrintEx("DsmpGetPath (DsmIds %p, DevNum %s): Path to be used for LeastBlocks is %p.\n",
                            DsmList,
							deviceInfo->SerialNumber,
                            failGroup);
							*/
        ...
        default: {
			/*
            TracePrintEx("DsmpGetPath (DsmIds %p, DevNum %s): Invalid LB Type %d set for group %p.\n",
                        DsmList,
						deviceInfo->SerialNumber,
                        groupEntry->LoadBalanceType,
                        groupEntry);
...
__Exit_DsmpGetPath:
	...
	if (failGroup) {
		...
		TracePrintEx("DsmpGetPath (DsmIds %p, DevNum %s): Exiting function with failGroup %p, pathID %p.\n",
			DsmList,
			deviceInfo->SerialNumber,
			failGroup,
			failGroup->PathId);
		...
		TracePrintEx("DsmpGetPath (DsmIds %p, DevNum %s): Exiting function with failGroup %p.\n",
			DsmList,
			deviceInfo->SerialNumber,
			failGroup);
	}
...
--*/
{
    ...
    TracePrintEx("DsmpMpioPassThroughPathCommand (Irp %p): Entering function.\n",
                Irp);
    ...
    TracePrintEx("DsmpMpioPassThroughPathCommand (Irp %p): Exiting function - IsMpioPassThruPathCmd %d.\n",
                Irp,
                isMPTPCommand);
...
--*/
{
    ...
    TracePrintEx("DsmpGetAnyActivePath (Group %p): Entering function.\n",
                Group);
	...
	for (inx = 0; inx < DSM_MAX_PATHS; inx++) {
		...
		if (existingDeviceInfo) {
			TracePrintEx("DsmpGetAnyActivePath (Group %p): DevInfo %p, DevNum %s, PathId %p, State %d, AluaState: %d, Usable %d, Init %d.\n",
				Group,
				existingDeviceInfo,
	...
	for (inx = 0; inx < DSM_MAX_PATHS; inx++) {
		...
		/*打桩模拟路径为优化，看是否会出现disk -1的问题*/
		/*if (existingDeviceInfo &&  existingDeviceInfo->State == DSM_DEV_ACTIVE_UNOPTIMIZED)
		{
			TracePrintEx("DsmpGetAnyActivePath (Group %p, DevInfo %p, DevNum %s, PathId %p): Modify dev state to DSM_DEV_ACTIVE_UNOPTIMIZED.\n",
				Group,
				existingDeviceInfo,
				existingDeviceInfo->SerialNumber,
				existingDeviceInfo->FailGroup->PathId);
    ...
    TracePrintEx("DsmpGetAnyActivePath (Group %p): Exiting function with DevInfo %p\n",
                Group,
                candidateDevInfo);
...
--*/
{
    ...
    TracePrintEx("DsmpGetActivePathToBeUsed (Group %p): Entering function.\n",
                Group);
    ...
    if (!deviceInfo && !Symmetric) {
        ...
        if ((deviceInfo != NULL) &&
            ...
            TracePrintEx("DsmpGetActivePathToBeUsed (Group %p): Using best alternative candidate device %p\n",
                        Group,
                        deviceInfo);
        } else {
            ...
            TracePrintEx("DsmpGetActivePathToBeUsed (Group %p): No active/alternative path available for group\n",
                        Group);
        }
    }

    TracePrintEx("DsmpGetActivePathToBeUsed (Group %p): Exiting function with devInfo %p.\n",
                Group,
                deviceInfo);
...
--*/
{
    ...
    TracePrintEx("DsmpFindStandbyPathToActivate(Group %p): Entering function.\n",
                Group);
    ...
    TracePrintEx("DsmpFindStandbyPathToActivate (Group %p): Exiting function with devInfo %p.\n",
                Group,
                candidateDevInfo);
...
--*/
{
    ...
    TracePrintEx("DsmpFindStandbyPathToActivateALUA (Group %p): Entering function.\n",
                Group);
    ...
    TracePrintEx("DsmpFindStandbyPathToActivateALUA (Group %p): Exiting function with devInfo %p.\n",
                Group,
                candidateDevInfo);
...
--*/
{
    ...
    TracePrintEx("DsmpSetLBForPathArrival (DevInfo %p): Entering function.\n",
                NewDeviceInfo);
    ...
    if (group->NumberDevices == 1) {
        ...
        TracePrintEx("DsmpSetLBForPathArrival (DevInfo %p): State changed to %d at %d\n",
                    NewDeviceInfo,
                    NewDeviceInfo->State,
                    __LINE__);
...
__Exit_DsmpSetLBForPathArrival:
    ...
    TracePrintEx("DsmpSetLBForPathArrival (DevInfo %p): Exiting function with status %x\n",
                NewDeviceInfo,
                status);
...
--*/
{
    ...
    TracePrintEx("DsmpSetLBForPathArrivalALUA (DevInfo %p): Entering function.\n",
                NewDeviceInfo);
    ...
    if (group->NumberDevices == 1) {
        ...
        if (NewDeviceInfo->State != DSM_DEV_ACTIVE_OPTIMIZED) {
            ...
            if (NewDeviceInfo->ALUASupport >= DSM_DEVINFO_ALUA_EXPLICIT) {
                ...
                if (NT_SUCCESS(status)) {
                    ...
                    //
                    TracePrintEx("DsmpSetLBForPathArrivalALUA (DevInfo %p): RTPG returned state = %x, ALUA state = %x.\n",
                                NewDeviceInfo,
                                NewDeviceInfo->State,
        ...
        TracePrintEx("DsmpSetLBForPathArrivalALUA (DevInfo %p): State changed to %d at %d\n",
                    NewDeviceInfo,
                    NewDeviceInfo->State,
                    __LINE__);
    ...
    if (NT_SUCCESS(status)) {
        ...
        TracePrintEx("DsmpSetLBForPathArrivalALUA (DevInfo %p): Trying to query ALUA state failed with %x\n",
                    NewDeviceInfo,
                    status);
    }
...
__Exit_DsmpSetLBForPathArrivalALUA:
    ...
    TracePrintEx("DsmpSetLBForPathArrivalALUA (DevInfo %p): Exiting function with status %x\n",
                NewDeviceInfo,
                status);
...
--*/
{
    ...
    TracePrintEx("DsmpSetLBForPathRemoval (DevInfo %p, DevNum %s, Group %p): Entering function.\n",
                RemovedDeviceInfo,
				RemovedDeviceInfo->SerialNumber,
                Group);
    ...
    if (Group == NULL) {
        ...
        TracePrintEx("DsmpSetLBForPathRemoval (DevInfo %p, DevNum %s): changed to state %d at %d\n",
                    RemovedDeviceInfo,
					RemovedDeviceInfo->SerialNumber,
    ...
    switch(group->LoadBalanceType) {
        ...
        case DSM_LB_ROUND_ROBIN_WITH_SUBSET: {
            ...
            if (!DsmpGetAnyActivePath(group, FALSE, NULL, SpecialHandlingFlag)) {
                ...
                if (deviceInfo) {
                    ...
                    TracePrintEx("DsmpSetLBForPathRemoval (DevInfo %p, DevNum %s): DevInfo %p changed to state %d at %d\n",
                                RemovedDeviceInfo,
								RemovedDeviceInfo->SerialNumber,
                                deviceInfo,
                                deviceInfo->State,
                ...
                TracePrintEx("DsmpSetLBForPathRemoval (DevInfo %p, DevNum %s): LB Policy FO/RRWS and other paths active, no path made active at %d\n",
                            RemovedDeviceInfo,
							RemovedDeviceInfo->SerialNumber,
                            __LINE__);
            }
        ...
        case DSM_LB_DYN_LEAST_QUEUE_DEPTH: {
            ...
            //
            TracePrintEx("DsmpSetLBForPathRemoval (DevInfo %p, DevNum %s): LB Policy RR, LWP or LQD, no path made active at %d\n",
                        RemovedDeviceInfo,
						RemovedDeviceInfo->SerialNumber,
                        __LINE__);
            break;
    ...
    if (deviceInfo != NULL) {
        ...
        TracePrintEx("DsmpSetLBForPathRemoval (DevInfo %p, DevNum %s): Removal: Updating PathToBeUsed in %p to %p\n",
                    RemovedDeviceInfo,
					RemovedDeviceInfo->SerialNumber,
                    group,
                    group->PathToBeUsed);
        ...
        TracePrintEx("DsmpSetLBForPathRemoval (DevInfo %p, DevNum %s): After remove No FOG available for group %p\n",
                    RemovedDeviceInfo,
					RemovedDeviceInfo->SerialNumber,
                    group);
    ...
    TracePrintEx("DsmpSetLBForPathRemoval (DevInfo %p, DevNum %s): Exiting function with status %x\n",
                RemovedDeviceInfo,
				RemovedDeviceInfo->SerialNumber,
                status);
...
--*/
{
    ...
    TracePrintEx("DsmpSetLBForPathRemovalALUA (DevInfo %p, DevNum %s, DevState %d, Group %p): Entering function.\n",
                RemovedDeviceInfo,
				RemovedDeviceInfo->SerialNumber,
				RemovedDeviceInfo->State,
                Group);
    ...
    if (Group == NULL) {
        ...
        TracePrintEx("DsmpSetLBForPathRemovalALUA (DevInfo %p, DevNum %s): changed to state %d at %d\n",
                    RemovedDeviceInfo,
					RemovedDeviceInfo->SerialNumber,
                    RemovedDeviceInfo->State,
                    __LINE__);
    ...
    if (group->LoadBalanceType < DSM_LB_FAILOVER ||
        ...
        if (!DsmpGetAnyActivePath(group, FALSE, NULL, SpecialHandlingFlag)) {
            ...
            if (deviceInfo) {
                ...
                if (sendTPG) {
                    ...
                    if (NT_SUCCESS(status)) {
                        ...
                        TracePrintEx("DsmpSetLBForPathRemovalALUA (DevInfo %p): Trying to query for ALUA state failed with status %x\n",
                                    RemovedDeviceInfo,
                                    status);
                    }
                } else {
                ...
                if (NT_SUCCESS(status)) {
                    ...
                    TracePrintEx("DsmpSetLBForPathRemovalALUA (DevInfo %p): Device %p changed to state %d at %d\n",
                                RemovedDeviceInfo,
                                deviceInfo,
                                deviceInfo->State,
                                __LINE__);
            ...
            TracePrintEx("DsmpSetLBForPathRemovalALUA (DevInfo %p): Other paths active, no path made active at %d\n",
                        RemovedDeviceInfo,
                        __LINE__);
        }
    ...
    if (deviceInfo != NULL) {
        ...
        TracePrintEx("DsmpSetLBForPathRemovalALUA (DevInfo %p): Removal: Updating PathToBeUsed in %p to %p\n",
                    RemovedDeviceInfo,
                    group,
                    group->PathToBeUsed);
    } else {

        TracePrintEx("DsmpSetLBForPathRemovalALUA (DevInfo %p): No active/alternative path available for group %p\n",
                    RemovedDeviceInfo,
                    group);
    ...
    TracePrintEx("DsmpSetLBForPathRemovalALUA (DevInfo %p): Exiting function with status %x.\n",
                RemovedDeviceInfo,
                status);
...
--*/
{
    ...
    TracePrintEx("DsmpSetLBForPathFailingALUA (DevInfo %p, DevNum %s): Entering function.\n",
                FailingDeviceInfo,
				FailingDeviceInfo->SerialNumber);
    ...
    if (MarkDevInfoFailed) {
        ...
        TracePrintEx("DsmpSetLBForPathFailingALUA (DevInfo %p, DevNum %s): changed to state %d at %d\n",
                    FailingDeviceInfo,
					FailingDeviceInfo->SerialNumber,
                                    ...
                                    if (completionContext) {
                                        ...
                                        if (targetPortGroupsInfo) {
                                            ...
                                            if (failDevInfoListEntry) {
                                                ...
                                                TracePrintEx("DsmpSetLBForPathFailingALUA (DevInfo %p, DevNum %s): Sending down TPG asynchronously for %p using devInfo %p (path %p).\n",
                                                            FailingDeviceInfo,
															FailingDeviceInfo->SerialNumber,
...
--*/
{
    ...
    TracePrintEx("DsmpPhase2ProcessPathFailingALUA (DevInfo %p, DevNum %s, PathId %p): Entering function.\n",
                deviceInfo,
				deviceInfo->SerialNumber,
				deviceInfo->FailGroup->PathId);
    ...
    if ((scsiStatus == SCSISTAT_GOOD) &&
        ...
        TracePrintEx("DsmpPhase2ProcessPathFailingALUA (DevInfo %p, DevNum %s): RTPG using path %p succeeded.\n",
                    deviceInfo,
					deviceInfo->SerialNumber,
                    deviceInfo->FailGroup->PathId);
        ...
        if ((context->NumberRetries)--) {
            ...
            TracePrintEx("DsmpPhase2ProcessPathFailingALUA (DevInfo %p, DevNum %s): Retrying check condition using path %p. Retries remaining %u.\n",
                        deviceInfo,
						deviceInfo->SerialNumber,
                        deviceInfo->FailGroup->PathId,
        ...
        TracePrintEx("DsmpPhase2ProcessPathFailingALUA (DevInfo %p, DevNum %s, PathId %p): NTStatus 0%x, ScsiStatus 0x%x.\n",
                    deviceInfo,
					deviceInfo->SerialNumber,
					deviceInfo->FailGroup->PathId,
                    status,
            ...
            for (index = 0; index < DSM_MAX_PATHS; index++) {
                ...
                if (devInfo) {
                    ...
                    if (devInfo->ALUAState == DSM_DEV_ACTIVE_OPTIMIZED) {
                        ...
                        if (!DsmpIsDeviceFailedState(devInfo->State)) {
							TracePrintEx("DsmpPhase2ProcessPathFailingALUA (DevInfo %p, DevNum %s, PathId %p): change state to %x.\n", devInfo->ALUAState);
                            devInfo->PreviousState = devInfo->State;
                            devInfo->State = devInfo->ALUAState;
                        }
                    }
...
__Exit_DsmpPhase2ProcessPathFailingALUA:
	...
	TracePrintEx("DsmpPhase2ProcessPathFailingALUA (DevInfo %p, DevNum %s, PathId %p): State %d, AluaState %d, LastKnownGoodState %d, Usable %d, init %d.\n",
		deviceInfo,
		deviceInfo->SerialNumber,
		deviceInfo->FailGroup->PathId,
		deviceInfo->State,
    ...
    TracePrintEx("DsmpPhase2ProcessPathFailingALUA (DevInfo %p, DevNum %s, PathId %p): Exiting function.\n",
                deviceInfo,
				deviceInfo->SerialNumber,
				deviceInfo->FailGroup->PathId);
