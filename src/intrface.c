...
--*/
{
	...
	TracePrintEx("DsmInquire (DevObj %p): Entering function.\n", TargetDevice);
	...
	TracePrintEx("DsmInquire (DevObj %p): Entering function DsmpGetStandardInquiryData.\n", TargetDevice);
    ...
    if (NT_SUCCESS(status)) {
        ...
        supported = DsmpCheckScsiCompliance(TargetDevice,
                                            ...
                                            Descriptor,
                                            DeviceIdList);
		TracePrintEx("DsmInquire (DevObj %p): is supported: %d.\n", supported);

    } else {
		TracePrintEx("DsmInquire (DevObj %p): Failed to get inquiry data with status %x.\n", TargetDevice, status);
        status = STATUS_NOT_SUPPORTED;
    ...
    if (!supported) {
        ...
        if (!supported) {
			TracePrintEx("DsmInquire (DevObj %p): Unsupported Device.\n", TargetDevice);
            status = STATUS_NOT_SUPPORTED;
            goto __Exit_DsmInquire;
        }
    }
	...
	TracePrintEx("DsmInquire (DevObj %p): Is %ws multiported.\n", TargetDevice, multiport ? L"" : L"not");
	...
	TracePrintEx("DsmInquire (DevObj %p): alua status: %d.\n", TargetDevice, alua);
	...
	if (!serialNumber) {
		TracePrintEx("DsmInquire (DevObj %p): serial number: NULL.\n", TargetDevice);
	}
	else {
		TracePrintEx("DsmInquire (DevObj %p): serial number: %s.\n", TargetDevice, serialNumber);
	}
	...
	TracePrintEx("DsmInquire (DevObj %p): device name: %s.\n", TargetDevice, deviceName);
    ...
    if (alua != DSM_DEVINFO_ALUA_NOT_SUPPORTED) {
		...
		TracePrintEx("DsmInquire (DevObj %p): DsmpReportTargetPortGroups.\n", TargetDevice);
	...
	TracePrintEx("DsmInquire (DevObj %p): hardwareId: %s.\n", TargetDevice, hardwareId);
    ...
    if (!group) {
        ...
        TracePrint((TRACE_LEVEL_INFORMATION,
                    ...
                    TargetDevice,
                    deviceInfo));
		TracePrintEx("DsmInquire (DevObj %p): First device %p in the group.\n", TargetDevice, deviceInfo);
        newGroup = TRUE;
        ...
        if (group) {
            ...
            if (!hardwareId) {
                ...
                TracePrint((TRACE_LEVEL_WARNING,
                            ...
                            TargetDevice,
                            deviceInfo));
				TracePrintEx("DsmInquire (DevObj %p): Failed to build a hardwareId for %p.\n", TargetDevice, deviceInfo);
            }
            ...
            TracePrint((TRACE_LEVEL_ERROR,
                        ...
                        TargetDevice,
                        deviceInfo));
			TracePrintEx("DsmInquire (DevObj %p): Failed to allocate Group Entry for %p.\n", TargetDevice, deviceInfo);
        ...
        TracePrint((TRACE_LEVEL_INFORMATION,
                    ...
                    group,
                    deviceInfo));
		TracePrintEx("DsmInquire (DevObj %p): Found group %p for device %p.\n", TargetDevice, deviceInfo);
        ...
        if (index < DSM_MAX_PATHS) {
            ...
            if (!((deviceInfo->ALUASupport == group->DeviceList[index]->ALUASupport) ||
				...
				TracePrintEx("DsmInquire (DevObj %p): Mismatch in device instances' ALUA support %d vs %d.\n", 
					TargetDevice, 
					deviceInfo->ALUASupport,
					group->DeviceList[index]->ALUASupport);
        ...
        if (NT_SUCCESS(status)) {
            ...
            if (NT_SUCCESS(status)) {
                ...
                *DsmIdentifier = deviceInfo;
                ...
                TracePrintEx("DsmInquire (DevObj %p): Added device %p to group %p.\n",
                            TargetDevice,
                            *DsmIdentifier,
                            group);
            } else {

                TracePrintEx("DsmInquire (DevObj %p): Failed to add device %p to group %p - status %x.\n",
                            TargetDevice,
                            deviceInfo,
                            group,
                            status);
    ...
    TracePrintEx("DsmInquire (DevObj %p): Device %p added. State %d, Desired State %d\n",
                TargetDevice,
                deviceInfo,
                deviceInfo->State,
...
__Exit_DsmInquire:
    ...
    TracePrint((TRACE_LEVEL_VERBOSE,
                ...
                TargetDevice,
                status));
	TracePrintEx("DsmInquire (DevObj %p): Exiting function with status %x.\n",
		TargetDevice,
		status);
...
--*/
{
    ...
    TracePrintEx("DsmCompareDevices (DevInfo %p, DevNum %s): Entering function - comparing with (DevInfo %p, DevNum %s).\n",
                deviceInfo0,
				deviceInfo0->SerialNumber,
                deviceInfo1,
				deviceInfo1->SerialNumber);
    ...
    if (serialNumber0 && serialNumber1) {
        ...
        TracePrintEx("DsmCompareDevices (DevInfo %p): Serialnumber not assigned for %p and\\or %p.\n",
                    DsmId1,
                    deviceInfo0,
                    deviceInfo1);
    }
    ...
    TracePrintEx("DsmCompareDevices (DevInfo %p): Exiting function with match = %!bool!.\n",
                DsmId1,
                match);
...
--*/
{
    ...
    TracePrintEx("DsmGetControllerInfo (DevInfo %p, DevNum %s): Entering function.\n",
                DsmId,
...
__Exit_DsmGetControllerInfo:
    ...
    TracePrintEx("DsmGetControllerInfo (DevInfo %p, DevNum %s): Exiting function with status %x.\n",
                DsmId,
				deviceInfo->SerialNumber,
                status);
...
--*/
{
    ...
    TracePrintEx("DsmSetDeviceInfo (DevInfo %p): Entering function.\n",
                DsmId);
    ...
    if (!TargetObject) {
        ...
        TracePrintEx("DsmSetDeviceInfo (DevInfo %p): No target object.\n",
                    deviceInfo);
    ...
    if (!NT_SUCCESS(status)) {
        ...
        TracePrintEx("DsmSetDeviceInfo (DevInfo %p): Failed to query LB policy from registry. Status %x.\n",
                    deviceInfo,
                    status);
    ...
    if (!explicitlySet) {
        ...
        if (NT_SUCCESS(status)) {
            ...
            if (NT_SUCCESS(status)) {
                ...
                TracePrintEx("DsmSetDeviceInfo (DevInfo %p): Failed to query Dsm overall LB policy from registry. Status %x.\n",
                            deviceInfo,
                            status);
            ...
            TracePrintEx("DsmSetDeviceInfo (DevInfo %p): Failed to query VID/PID LB policy from registry. Status %x.\n",
                        deviceInfo,
                        status);
    ...
    *PathId = ((PVOID)((ULONG_PTR)(pathId)));
    ...
    if (!failGroup) {
        ...
        if (failGroup) {
            ...
            TracePrintEx("DsmSetDeviceInfo (DevInfo %p): Failed to build FO Group.\n",
                        DsmId);
    ...
    TracePrintEx("DsmSetDeviceInfo (DevInfo %p, DevNum %s): PathWeight %x, DesiredState %x, State %x, PrevState %x, IsSymmetric %x.\n",
                deviceInfo,
				deviceInfo->SerialNumber,
    ...
    if (NT_SUCCESS(status)) {
        ...
        TracePrintEx("DsmSetDeviceInfo (DevInfo %p): No path associated with instance. Changing state from %u to %u.\n",
                    deviceInfo,
                    deviceInfo->PreviousState,
                    deviceInfo->State);
        ...
        if (failGroup->Count == 0) {
            ...
            TracePrintEx("DsmSetDeviceInfo (DevInfo %p): Removing FOGroup %p with path %p. Count of FOGroups %d.\n",
                        DsmId,
                        failGroup,
                        failGroup->PathId,
...
__Exit_DsmSetDeviceInfo:
    ...
    TracePrintEx("DsmSetDeviceInfo (DevInfo %p): Exiting function with status %x.\n",
                DsmId,
                status);
...
--*/
{
    ...
    TracePrintEx("DsmIsPathActive (DevInfo %p, DevNum %s, PathID %p): Entering function.\n",
                DsmId,
				deviceInfo->SerialNumber,
				PathId);
	/*
	TracePrintEx("DsmIsPathActive (DevInfo %p, DevNum %s, PathId %p): State %d, AluaState %d, LastKnownGoodState %d, Usable %d, init %d.\n",
		DsmId,
		deviceInfo->SerialNumber,
		PathId,
		deviceInfo->State,
		deviceInfo->ALUAState,
		deviceInfo->LastKnownGoodState,
		deviceInfo->Usable,
		deviceInfo->Initialized);
		*/

	TracePrintEx("DsmIsPathActive: Dump All paths:");
	for (inx = 0; inx < DSM_MAX_PATHS; inx++) {
		existingDeviceInfo = group->DeviceList[inx];
		if (existingDeviceInfo) {
			TracePrintEx("DsmIsPathActive (DevInfo %p, DevNum %s, PathId %p): State %d, AluaState: %d, LastKnownGoodState %d, Usable %d, Init %d.\n",
				existingDeviceInfo,
				existingDeviceInfo->SerialNumber,
				existingDeviceInfo->FailGroup->PathId,
				existingDeviceInfo->State,
				existingDeviceInfo->ALUAState,
				existingDeviceInfo->LastKnownGoodState,
				existingDeviceInfo->Usable,
				existingDeviceInfo->Initialized
			);
		}
	}
    ...
    if (group->PRKeyValid) {
        ...
        if (NT_SUCCESS(prRegStatus)) {
            ...
            TracePrintEx("DsmIsPathActive (DevInfo %p, DevNum %s, PathID %p): Failed (status %x) to register PR key\n",
                        deviceInfo,
						deviceInfo->SerialNumber,
						PathId,
                        prRegStatus);
    ...
    if ((foGroup) &&
        ...
        TracePrintEx("DsmIsPathActive (DevInfo %p, DevNum %s, PathID %p): Path %p is usable.\n",
                    DsmId,
					deviceInfo->SerialNumber,
					PathId,
                    PathId);
...
/*
        TracePrintEx("DsmIsPathActive (DevInfo %p, DevNum %s, PathId %p): State set to %d\n",
                    deviceInfo,
					deviceInfo->SerialNumber,
					PathId,
                    deviceInfo->State)
*/
        ...
        if (group->PathToBeUsed == NULL) {
            ...
            TracePrintEx("DsmIsPathActive (DevInfo %p, DevNum %s, PathID %p): Will set PathToBeUsed for %p\n",
                        deviceInfo,
						deviceInfo->SerialNumber,
						PathId,
                        group);
...
/*
    ...
    TracePrintEx("DsmIsPathActive (DevInfo %p, DevNum %s, PathId %p): Exiting function with retVal = %d.\n",
                DsmId,
				deviceInfo->SerialNumber,
				PathId,
                retVal);
...

{
    ...
    TracePrintEx("DsmPathVerify (DevInfo %p, DevNum %s, PathId %p): Entering function.\n",
                DsmId,
				deviceInfo->SerialNumber, 
				PathId);
	/*
	TracePrintEx("DsmPathVerify (DevInfo %p, DevNum %s, PathId %p): State %d, AluaState %d, LastKnownGoodState %d, Usable %d, init %d.\n",
		DsmId,
		deviceInfo->SerialNumber,
		PathId,
		deviceInfo->State,
		...
		deviceInfo->Initialized);
		*/
	TracePrintEx("DsmPathVerify: Dump All paths:");
	for (inx = 0; inx < DSM_MAX_PATHS; inx++) {
		existingDeviceInfo = group->DeviceList[inx];
		if (existingDeviceInfo) {
			TracePrintEx("DsmPathVerify (DevInfo %p, DevNum %s, PathId %p): State %d, AluaState: %d, LastKnownGoodState %d, Usable %d, Init %d.\n",
    ...
    if (DsmpIsDeviceInitialized(deviceInfo)) {
        ...
        if (foGroup) {
            ...
            // torn stuff down.
            //
            TracePrintEx("DsmPathVerify (DevInfo %p, DevNum %s, PathID %p): Failed to find failover group for path %p.\n",
                        DsmId,
						deviceInfo->SerialNumber,
						PathId,
                        PathId);
        ...
        if (NT_SUCCESS(status)) {
            ...
            if (found) {
                ...
                if (deviceInfo->ALUASupport == DSM_DEVINFO_ALUA_NOT_SUPPORTED) {
                    ...
                    TracePrintEx("DsmPathVerify (DevInfo %p, DevNum %s): Sending TUR using %p to verify path %p.\n",
                                DsmId,
								deviceInfo->SerialNumber,
                                deviceInfo,
                    ...
                    if (deviceInfo->IgnorePathVerify) {
                        ...
                        TracePrintEx("DsmPathVerify (DevInfo %p, DevNum %s): Returning success immediately since RTPG was already just sent.\n",
                                    DsmId,
									deviceInfo->SerialNumber);
                        ...
                        TracePrintEx("DsmPathVerify (DevInfo %p, DevNum %s): Sending RTPG using %p to verify path %p.\n",
                                    DsmId,
									deviceInfo->SerialNumber,
                                    deviceInfo,
                                    deviceInfo->FailGroup->PathId);
...
/*
    ...
    TracePrintEx("DsmPathVerify (DevInfo %p, DevNum %s, PathID %p): Exiting function with status %x.\n",
                DsmId,
				deviceInfo->SerialNumber,
				PathId,
                status);
...
--*/
{
	...
	TracePrintEx("DsmInvalidatePath (PathId %p): Entering function.\n", PathId);
	TracePrintEx("DsmInvalidatePath (PathId %p): %x.\n", PathId, ErrorMask);
    ...
    *NewPathId = NULL;
    ...
    TracePrintEx("DsmInvalidatePath (PathId %p): Context %p, FOG %p failing. Count %d\n",
                PathId,
                DsmContext,
                failGroup,
				failGroup->Count);
    ...
    if (failGroup->Count == 0) {
        ...
        if (!newPath) {
            ...
            //
			TracePrintEx("DsmInvalidatePath (PathId %p): Failed to find new path using zombie group list.\n",
				PathId);
        }
    ...
    if (!newPath) {
        ...
        //
		TracePrintEx("DsmInvalidatePath (PathId %p): No valid path found.\n", PathId);
        status = STATUS_NO_SUCH_DEVICE;
        ...
        *NewPathId = newPath->PathId;
		...
		TracePrintEx("DsmInvalidatePath (PathId %p): Returning %p as newPath.\n",
			PathId,
			newPath->PathId);
    }
...
__Exit_DsmInvalidatePath:
	...
	TracePrintEx("DsmInvalidatePath (PathId %p): Exiting function with status %x.\n",
		PathId,
		status);
    return status;
}
...
--*/
{
    ...
    TracePrintEx("DsmMoveDevice (DsmIds %p): Entering function - DsmContext %p MPIOPath (%p) SuggestedPath %p.\n",
                DsmIds,
                DsmContext,
                MPIOPath,
                SuggestedPath);
    ...
    TracePrintEx("DsmMoveDevice (DsmIds %p): Exiting function with status %x.\n",
                DsmIds,
                status);
...

{
	...
	TracePrintEx("DsmRemovePending (DevInfo %p, DevNum %s, PathID %p): Entering function.\n",
		DsmId,
		deviceInfo->SerialNumber,
		deviceInfo->FailGroup->PathId);
	TracePrintEx("DsmRemovePending (DevInfo %p, DevNum %s, PathID %p): LastKnownGoodState： %d, PreviousState: %d, State: %d.\n",
		DsmId,
		deviceInfo->SerialNumber,
		deviceInfo->FailGroup->PathId,
		deviceInfo->LastKnownGoodState,
	...
	TracePrintEx("DsmRemovePending (DevInfo %p, DevNum %s, PathID %p):  Exiting function.\n",
		DsmId,
		deviceInfo->SerialNumber,
		deviceInfo->FailGroup->PathId);
...

{
	...
	TracePrintEx("DsmRemoveDevice(DevInfo %p, DevNum: %s, PathId %p) : Entering function.\n",
		DsmId,
		deviceInfo->SerialNumber,
		PathId);
    ...
    TracePrintEx("DsmRemoveDevice (DevInfo %p, PathId %p): Exiting function.\n",
                DsmId,
				PathId);
...

{
    ...
    TracePrintEx("DsmRemovePath (PathId %p): Entering function.\n",
                PathId);
    ...
    if (failGroup) {
        ...
        if (failGroup->Count == 0) {
            ...
            if (InterlockedCompareExchange(&failGroup->NumberOfRequestsInFlight, 0, 0) > 0) {
                ...
                TracePrintEx("DsmRemovePath (PathId %p): Outstanding requests %d. Moving FOGroup %p with path %p to stale path list.\n",
                            PathId,
                ...
                TracePrintEx("DsmRemovePath (PathId %p): Removing FOGroup %p with path %p. Count of FOGroups %d.\n",
                            PathId,
                            failGroup,
                            failGroup->PathId,
                            DsmContext->NumberFOGroups);
            ...
            TracePrintEx("DsmRemovePath (PathId %p): Count %d. Not removing FOGroup %p.\n",
                        PathId,
                        failGroup->Count,
                        failGroup);
        ...
        //
        TracePrintEx("DsmRemovePath (PathId %p): Did not find the FO group.\n",
                    PathId);
    ...
    TracePrintEx("DsmRemovePath (PathId %p): Exiting function.\n",
                PathId);
...

{
	...
	/*
	TracePrintEx("DsmLBGetPath (DsmIds %p, Srb %p): Entering function.\n", DsmList, Srb);
	*/
    ...
    if (Srb) {
		...
		/*
		TracePrintEx("DsmLBGetPath(DsmIds %p, Srb %p) : opcode: 0x%x, dataLen: %d, Status %x, SrbStatus %x.\n",
			DsmList,
			Srb,
			cdb->CDB6READWRITE.OperationCode,
			Srb->DataTransferLength,
			*Status,
			Srb->SrbStatus
		);
		TracePrintHexDump(&Srb->Cdb[0], 16);
		*/
    }
    ...
    if (!failGroup) {
        ...
        if (deviceInfo) {
			...
			/*
            TracePrintEx("DsmLBGetPath (DsmIds %p): Couldn't find FOG but FO in progress, so returning devInfo %p (FOG %p path %p).\n",
                        DsmList,
                        deviceInfo,
                        deviceInfo->FailGroup,
                        deviceInfo->FailGroup->PathId);
			*/
            ...
            if (InterlockedCompareExchange((LONG volatile*)&group->InFlightRTPG, 0, 0)) {
                ...
                if (deviceInfo) {
					...
					/*
                    TracePrintEx("DsmLBGetPath (DsmIds %p): Couldn't find FOG but RTPG inflight, so returning devInfo %p (FOG %p path %p).\n",
                                DsmList,
                                deviceInfo,
                                deviceInfo->FailGroup,
                                deviceInfo->FailGroup->PathId);
					...
					/*
                    TracePrintEx("DsmLBGetPath (DsmIds %p): Couldn't find FOG but RTPG inflight, even then couldn't find alternative devInfo.\n",
                                DsmList);
								*/
                }
            }
    ...
    if (failGroup) {
            ...
            if (oldPath) {
                ...
                if (DsmpDecrementCounters(oldPath, Srb)) {
                    ...
                    if (oldPath->State == DSM_FG_PENDING_REMOVE) {
						...
						/*
                        TracePrintEx("DsmLBGetPath (DsmIds %p): Removing FOGroup %p with path %p.\n",
                                    DsmList,
                                    oldPath,
                                    oldPath->PathId);
						*/
        ...
        *Status = STATUS_NO_SUCH_DEVICE;
		...
		/*
        TracePrintEx("DsmLBGetPath (DsmIds %p): Failed to get FO group in LBGetPath.\n",
                    DsmList);
					*/
    }
...
__Exit_DsmLBGetPath:
	/*
	TracePrintEx("DsmLBGetPath (DsmIds %p, Srb %p): Exiting function returning path %p for request %p.\n",
		DsmList,
		Srb,
		newPath,
		Srb);
...
--*/
{
	...
	TracePrintEx("DsmCategorizeRequest (DsmIds %p): Entering function.\n",
		DsmIds);
	...
	if (Srb) {
		...
		TracePrintEx("DsmCategorizeRequest(DsmIds %p, Srb %p) : opcode: 0x%x, dataLen: %d, Status %x, SrbStatus %x.\n",
			DsmIds,
			Srb,
			cdb->CDB6READWRITE.OperationCode,
			Srb->DataTransferLength,
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (!*PathId) {
            TracePrintEx("DsmCategorizeRequest (DsmIds %p): DSM_PATH_SET didn't return a path.\n",
                        DsmIds);
        }
...
__Exit_DsmCategorizeRequest:
    ...
    TracePrintEx("DsmCategorizeRequest (DsmIds %p): Exiting function with categorization %x.\n",
                DsmIds,
                dsmStatus);
...
--*/
{
    ...
    TracePrintEx("DsmBroadcastRequest (DsmIds %p): Entering function.\n",
                DsmIds);
    ...
    TracePrintEx("DsmBroadcastReqeust (DsmIds %p): Exiting function with status %x.\n",
                DsmIds,
                status);
...
--*/
{
    ...
    TracePrintEx("DsmSetCompletion (DevInfo %p, DevNum %s): Entering function.\n",
                DsmId,
				deviceInfo->SerialNumber);
    ...
    TracePrintEx("DsmSetCompletion (DevInfo %p, DevNum %s): Exiting function.\n",
                DsmId,
				deviceInfo->SerialNumber);
...
--*/
{
	...
	TracePrintEx("DsmInterpretError(DevInfo %p, Devnum %s, Srb %p) : Entering function.\n",
		DsmId,
		(const char*)deviceInfo->SerialNumber,
		Srb);
	TracePrintEx("DsmInterpretError(DevInfo %p, Devnum %s, Srb %p) : opcode: 0x%x, dataLen: %d, Status %x, SrbStatus %x, ScsiStatus %x.\n",
		DsmId,
		(const char*)deviceInfo->SerialNumber,
		Srb,
		cdb->CDB6READWRITE.OperationCode,
    ...
    *RetryInterval = MAXLONG;
    ...
    if ((scsiStatus == SCSISTAT_RESERVATION_CONFLICT) ||
        ...
        TracePrintEx("DsmInterpretError (DevInfo %p, DevNum %s): Srb %p. Either busy or res. conflict (%x %x).\n",
                    DsmId,
					deviceInfo->SerialNumber,
                    Srb,
                    scsiStatus,
	...
	/* 如果是 Sense 数据有效，则将相关的信息 dump 出来 */
	if (validSense) {
		TracePrintEx("DsmInterpretError(DevInfo %p, Devnum %s, Srb %p) : senseData: %p, senseDataLength: %d, senseKey: %x, addSenseCode: %x, addSenseCodeQualifier: %x.\n", 
			DsmId,
			(const char*)deviceInfo->SerialNumber,
			Srb,
			senseData,
		...
		TracePrintEx("Sense Data:\n");
		TracePrintHexDump(senseData, senseDataLength);
	}
    ...
    if (validSense) {
        if (senseKey == SCSI_SENSE_NOT_READY &&
            ...
            TracePrintEx("DsmInterpretError (DevInfo %p, DevNum %s): Temporary resource exhaustion. Fail Srb %p.\n",
                        DsmId,
						deviceInfo->SerialNumber,
                        Srb);
            ...
            TracePrintEx("DsmInterpretError (DevInfo %p, DevNum %s): Permanent resource exhaustion. Fail Srb %p.\n",
                        DsmId,
						deviceInfo->SerialNumber,
                        Srb);
    ...
    switch (*Status) {
        ...
        case STATUS_IO_DEVICE_ERROR: {
            ...
            if (Srb->SrbStatus & SRB_STATUS_AUTOSENSE_VALID) {
                ...
                TracePrintEx("DsmInterpretError (DevInfo %p): Failing request. STATUS_IO_DEVICE_ERROR. SrbStatus (%x) ScsiStatus (%x).\n",
                            DsmId,
                            Srb->SrbStatus,
                            scsiStatus);
            }
    ...
    *Retry = retry;
	...
	TracePrintEx("DsmInterpretError(DevInfo %p, Devnum %s, Srb %p) : Exiting function returning errorMask %x.\n", 
		DsmId,
		(const char*)deviceInfo->SerialNumber,
		Srb,
		errorMask);

	TracePrintEx("DsmInterpretError(DevInfo %p, Devnum %s, Srb %p) : Status %x, SrbStatus %x, ScsiStatus %x, Retry %d, RetryInterval %d.\n",
		DsmId,
		(const char*)deviceInfo->SerialNumber,
		Srb,
		*Status,
...
--*/
{
	...
	TracePrintEx("DsmDeviceNotUsed (DevInfo %p, DevNum %s): Entering function.\n",
		DsmId, 
		deviceInfo->SerialNumber);
	...
	TracePrintEx("DsmDeviceNotUsed (DevInfo %p, DevNum %s): Exiting function.\n",
		DsmId,
		deviceInfo->SerialNumber);
...

{
    ...
    TracePrintEx("DsmUnload (DsmCtxt %p): Entering function.\n",
                DsmContext);
    ...
    TracePrintEx("DsmUnload (DsmCtxt %p): Exiting function.\n",
                tempAddress);
